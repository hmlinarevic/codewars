...
const getFullSet = (events) => {
  const set = [];
  for (let i = 3; i < events; i *= 3) {
    set.push(3);
  }
  return set;
};
...
const eqDice = (set) => {
  const numOfEvents = set.reduce((total, dice) => (total *= dice), 1); // 36
  const dices = { min: 3, max: 20 };

  const fullSet = getFullSet(numOfEvents); // 3
  let currentSet = [3, 3];
  let eqDiceSets = 0;

  while (currentSet.length <= fullSet.length) {
    // currentSet [3, 3]

    // startLooping through current set and find dices that equal num of events
    // for (let i = 0; i < currentSet.length; i++) {

    // }

    // max dice value in a set is 20

    // when dices found save increase count
    eqDiceSets++;

    currentSet.push(3); // [3, 3, 3]
  }

  return eqDiceSets;
};

eqDice([6, 6]); // --> 3 ([4, 3, 3], [12, 3], [9, 4])
